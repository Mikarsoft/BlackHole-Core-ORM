...
namespace BlackHole.DataProviders
{
    internal class SqLiteDataProvider : IDataProvider
    {
        ...
        private async Task<G?> ExecuteEntryScalarAsync<T, G>(string commandText, T entry)
        {
            ...
            {
                Task.Factory.StartNew(() => _loggerService.CreateErrorLogs($"InsertAsync_{TableName}", commandText, ex.Message, ex.ToString())).Start();
                return default;
            }
        }
        ...
        public bool ExecuteEntry<T>(string commandText, T entry)
        {
            ...
            {
                Task.Factory.StartNew(() => _loggerService.CreateErrorLogs($"Insert_{TableName}", commandText, ex.Message, ex.ToString())).Start();
                return false;
            }
        }
