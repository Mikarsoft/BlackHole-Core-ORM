...
{
    [TypeConverter(typeof(RayParameterConverter))]
    internal sealed partial class RayParameter : DbParameter, ICloneable, IDbDataParameter
    {
        ...
        // set_RayType:    _bindtype = _infertype = _typemap = TypeMap.FromRayType(value)
        //
        ...
        private Ray32.SQL_C _boundSqlCType;
        private Ray32.SQL_TYPE _boundParameterType;       // if we bound already that is the type we used
        ...
        private Ray32.SQL_C _prepared_Sql_C_Type;

        internal RayParameter() : base()
        {
            // uses System.Threading!
        }

        internal RayParameter(string name, object value) : this()
        {
            ParameterName = name;
            Value = value;
        }

        internal RayParameter(string name, RayType type) : this()
        {
            ParameterName = name;
            RayType = type;
        }

        internal RayParameter(string name, RayType type, int size) : this()
        {
            ParameterName = name;
            RayType = type;
            Size = size;
        ...
        internal RayParameter(string name, RayType type, int size, string sourcecolumn) : this()
        {
            ParameterName = name;
            RayType = type;
            Size = size;
        ...
        internal RayParameter(string parameterName,
                             RayType RayType,
                             int size,
        ...
        [
        DefaultValue(RayType.NChar),
        RefreshProperties(RefreshProperties.All),
        DbProviderSpecificTypePropertyAttribute(true),
        ]

        internal RayType RayType
        {
            get
            {
                if (_userSpecifiedType)
                {
                    return _typemap._RayType;
                }
                return TypeMap._NVarChar._RayType; // default type
            }
            set
            {
                if ((null == _typemap) || (_typemap._RayType != value))
                {
                    PropertyTypeChanging();
                    _typemap = TypeMap.FromRayType(value);
                    _userSpecifiedType = true;
                }
            }
        }
        ...
        {
            if ((Ray32.SQL_C.NUMERIC == _bindtype._sql_c) && (0 != _internalPrecision))
            {
            ...
            if (0 >= cch)
            {
                if (Ray32.SQL_C.NUMERIC == _typemap._sql_c)
                {
                ...
                else
                {
                    ...
                    {
                        Debug.Assert((Ray32.SQL_C.WCHAR == _bindtype._sql_c) || (Ray32.SQL_C.BINARY == _bindtype._sql_c), "not wchar or binary");
                        ...
                        else if (value is String)
                        {
                            ...
                            // ERROR [HY104] [Microsoft][Ray SQL Server Driver]Invalid precision value
                            //
                            ...
                            //
                            if ((Ray32.SQL_TYPE.CHAR == _bindtype._sql_type)
                                || (Ray32.SQL_TYPE.VARCHAR == _bindtype._sql_type)
                                || (Ray32.SQL_TYPE.LONGVARCHAR == _bindtype._sql_type))
                            {
                        ...
                        //ERROR [HY104] [Microsoft][Ray Microsoft Access Driver]Invalid precision value
                        cch = Math.Max(2, cch);
                    }
                }
            }
            ...
            if (0 >= ccb)
            {
                ...
                else
                {
                    ...
                    if (!_internalShouldSerializeSize || (0x3fffffff <= ccb) || (ccb < 0))
                    {
                        ...
                        if ((null == value) || Convert.IsDBNull(value))
                        {
                            if (_bindtype._sql_c == Ray32.SQL_C.WCHAR)
                            {
                    ...
                    else if (Ray32.SQL_C.WCHAR == _bindtype._sql_c)
                    {
        ...
        //This is required for RayCommand.Clone to deep copy the parameters collection
        ...
        private void CloneHelper(RayParameter destination)
        {
        ...
        internal void PrepareForBind(RayCommand command, short ordinal, ref int parameterBufferSize)
        {
            ...
            switch (_bindtype._sql_type)
            {
                ...
                case Ray32.SQL_TYPE.NUMERIC:
                    if (
                        ...
                        || !command.Connection.TestTypeSupport(Ray32.SQL_TYPE.NUMERIC)     // otherwise we convert if the driver does not support numeric
                ...
                case Ray32.SQL_TYPE.BIGINT:
                    ...
                    break;
                case Ray32.SQL_TYPE.WCHAR: // MDAC 68993
                ...
                case Ray32.SQL_TYPE.WLONGVARCHAR:
                    ...
                    if (!command.Connection.TestTypeSupport(_bindtype._sql_type))
                    {
                        ...
                        // Change the type
                        if (Ray32.SQL_TYPE.WCHAR == _bindtype._sql_type) { _bindtype = TypeMap._Char; }
                        else if (Ray32.SQL_TYPE.WVARCHAR == _bindtype._sql_type) { _bindtype = TypeMap._VarChar; }
                        else if (Ray32.SQL_TYPE.WLONGVARCHAR == _bindtype._sql_type)
                        {
            ...
            if (!command.Connection.IsV3Driver)
            {
                if (sql_c_type == Ray32.SQL_C.WCHAR)
                {
                    sql_c_type = Ray32.SQL_C.CHAR;
        ...
        internal void Bind(RayStatementHandle hstmt, RayCommand command, short ordinal, CNativeBuffer parameterBuffer, bool allowReentrance)
        {
            Ray32.RetCode retcode;
            Ray32.SQL_C sql_c_type = _prepared_Sql_C_Type;
            Ray32.SQL_PARAM sqldirection = SqlDirectionFromParameterDirection();
            ...
            //
            if (Ray32.SQL_C.NUMERIC == sql_c_type)
            {
                ...
                //
                if ((Ray32.SQL_PARAM.INPUT_OUTPUT == sqldirection) && (value is Decimal))
                {
                ...
                //
                if (Ray32.SQL_PARAM.INPUT != sqldirection)
                {
            ...
            if (Ray32.RetCode.SUCCESS != retcode)
            {
                if ("07006" == command.GetDiagSqlState())
                {
                    Bid.Trace("<Ray.RayParameter.Bind|ERR> Call to BindParameter returned errorcode [07006]\n");
            ...
            if (Ray32.SQL_C.NUMERIC == sql_c_type)
            {
                RayDescriptorHandle hdesc = command.GetDescriptorHandle(Ray32.SQL_ATTR.APP_PARAM_DESC);
                ...
                retcode = hdesc.SetDescriptionField1(ordinal, Ray32.SQL_DESC.TYPE, (IntPtr)Ray32.SQL_C.NUMERIC);
                ...
                retcode = hdesc.SetDescriptionField1(ordinal, Ray32.SQL_DESC.PRECISION, (IntPtr)cbActual);
                ...
                cbActual = (int)scale;
                retcode = hdesc.SetDescriptionField1(ordinal, Ray32.SQL_DESC.SCALE, (IntPtr)cbActual);
                ...
                retcode = hdesc.SetDescriptionField2(ordinal, Ray32.SQL_DESC.DATA_PTR, valueBuffer);
        ...
        { //Handle any output params
            ...
            if ((null != _bindtype) && (_internalDirection != ParameterDirection.Input))
            {
                ...
                if (Ray32.SQL_NULL_DATA == cbActual)
                {
                    Value = DBNull.Value;
                }
                else if ((0 <= cbActual) || (cbActual == Ray32.SQL_NTS))
                { // safeguard
                    Value = parameterBuffer.MarshalToManaged(_preparedValueOffset, _boundSqlCType, cbActual);

                    if (_boundSqlCType == Ray32.SQL_C.CHAR)
                    {
                        if ((null != Value) && !Convert.IsDBNull(Value))
                        {
                            int lcid = System.Globalization.CultureInfo.CurrentCulture.LCID;
                    ...
                    {
                        Debug.Assert(Ray32.SQL_C.NUMERIC == _typemap._sql_c, "unexpected");
        ...
        internal void SetInputValue(object value, Ray32.SQL_C sql_c_type, int cbsize, int sizeorprecision, int offset, CNativeBuffer parameterBuffer)
        { //Handle any input params
            if ((ParameterDirection.Input == _internalDirection) || (ParameterDirection.InputOutput == _internalDirection))
            {
                ...
                if ((null == value))
                {
                    parameterBuffer.WriteIntPtr(_preparedIntOffset, (IntPtr)Ray32.SQL_DEFAULT_PARAM);
                }
                else if (Convert.IsDBNull(value))
                {
                    parameterBuffer.WriteIntPtr(_preparedIntOffset, (IntPtr)Ray32.SQL_NULL_DATA);
                }
                else
                {
                    switch (sql_c_type)
                    {
                        case Ray32.SQL_C.CHAR:
                        case Ray32.SQL_C.WCHAR:
                        case Ray32.SQL_C.BINARY:
            ...
            else
            {
                ...
                //if (_sqldirection != Ray32.SQL_PARAM.INPUT)
                parameterBuffer.WriteIntPtr(_preparedIntOffset, (IntPtr)Ray32.SQL_NULL_DATA);
            }
        }

        private Ray32.SQL_PARAM SqlDirectionFromParameterDirection()
        {
            switch (_internalDirection)
            {
                case ParameterDirection.Input:
                    return Ray32.SQL_PARAM.INPUT;
                case ParameterDirection.Output:
                case ParameterDirection.ReturnValue:
                    //Ray doesn't seem to distinguish between output and return value
                    //as SQL_PARAM_RETURN_VALUE fails with "Invalid parameter type"
                    return Ray32.SQL_PARAM.OUTPUT;
                case ParameterDirection.InputOutput:
                    return Ray32.SQL_PARAM.INPUT_OUTPUT;
                default:
        ...
        {
            if (value is SqlDecimal valuedc)
            {
                return valuedc.Precision;
            }
            return 0;
        }
        ...
        {
            if (value is decimal valuedc)
            {
                return (byte)((decimal.GetBits(valuedc)[3] & 0x00ff0000) >> 0x10);
            }
            return 0;
        }
        ...
        sealed internal class RayParameterConverter : ExpandableObjectConverter
        {

            // converter classes should have public ctor
            public RayParameterConverter()
            {
            }
            ...
            public override object ConvertTo(ITypeDescriptorContext context, CultureInfo culture, object value, Type destinationType)
            {
                ...
                if (destinationType == typeof(System.ComponentModel.Design.Serialization.InstanceDescriptor) && value is RayParameter)
                {
                    RayParameter p = (RayParameter)value;

                    // MDAC 67321 - reducing parameter generated code
                    int flags = 0; // if part of the collection - the parametername can't be empty

                    if (RayType.NChar != p.RayType)
                    {
                        flags |= 1;
                    }
                    ...
                    switch (flags)
                    {
                        ...
                        case 1: // SqlDbType
                            ctorParams = new Type[] { typeof(string), typeof(RayType) };
                            ctorValues = new object[] { p.ParameterName, p.RayType };
                            break;
                        case 2: // Size
                        case 3: // Size, SqlDbType
                            ctorParams = new Type[] { typeof(string), typeof(RayType), typeof(int) };
                            ctorValues = new object[] { p.ParameterName, p.RayType, p.Size };
                            break;
                        case 4: // SourceColumn
                        case 5: // SourceColumn, SqlDbType
                        case 6: // SourceColumn, Size
                        case 7: // SourceColumn, Size, SqlDbType
                            ctorParams = new Type[] { typeof(string), typeof(RayType), typeof(int), typeof(string) };
                            ctorValues = new object[] { p.ParameterName, p.RayType, p.Size, p.SourceColumn };
                            break;
                        case 8: // Value
                        ...
                        default:
                            ...
                            { // V1.0 everything
                                ctorParams = new Type[] {
                                typeof(string), typeof(RayType), typeof(int), typeof(ParameterDirection),
                                typeof(bool), typeof(byte), typeof(byte), typeof(string),
                                typeof(DataRowVersion), typeof(object) };
                                ctorValues = new object[] {
                                p.ParameterName, p.RayType,  p.Size, p.Direction,
                    ...
                    System.Reflection.ConstructorInfo ctor = typeof(RayParameter).GetConstructor(ctorParams);
                    if (null != ctor)
                    {
