        ...
        internal bool HasBlankPassword
        {
            get
            {
                if (!ConvertValueToIntegratedSecurity())
                {
                    ...
                    {
                        return false; //ADP.IsEmpty((string)_parsetable[KEY.Password]);
                    }
                    else
                    if (_parsetable.ContainsKey(SYNONYM.Pwd))
                    {
                        return false;//ADP.IsEmpty((string)_parsetable[SYNONYM.Pwd]); // MDAC 83097
                    }
                    else
                    {
                        return (_parsetable.ContainsKey(KEY.User_ID)) && //!ADP.IsEmpty((string)_parsetable[KEY.User_ID])) || 
