...
{
    public sealed class RayDataReader : DbDataReader
    {
        ...
        internal RayDataReader(RayCommand command, CMDWrapper cmdWrapper, CommandBehavior commandbehavior)
        {
            RayConnection.VerifyExecutePermission();

            Debug.Assert(command != null, "Command null on RayDataReader ctor");
        ...
        private RayStatementHandle StatementHandle
        {
            get
            {
        ...
        private RayStatementHandle KeyInfoStatementHandle
        {
        ...
        override public int FieldCount
        {
            get
            {
                ...
                {
                    Int16 cColsAffected;
                    Ray32.RetCode retcode = this.FieldCountNoThrow(out cColsAffected);
        ...
        internal Ray32.RetCode FieldCountNoThrow(out Int16 cColsAffected)
        {
            ...
            Ray32.RetCode retcode = StatementHandle.NumberOfResultColumns(out cColsAffected);
            if (retcode == Ray32.RetCode.SUCCESS)
            {
                _hiddenColumns = 0;
        ...
        private SQLLEN GetRowCount()
        {
            if (!IsClosed)
            {
                ...
                Ray32.RetCode retcode = StatementHandle.RowCount(out cRowsAffected);
            ...
            if (null != wrapper && wrapper.StatementHandle != null)
            {
                ...
                if (IsNonCancelingCommand)
                {
                    ...
                    if (null != command)
                    {
                        ...
                        wrapper.FreeStatementHandle(Ray32.STMT.CLOSE);
                        command.CloseFromDataReader();
                    }
                }
                wrapper.FreeKeyInfoStatementHandle(Ray32.STMT.CLOSE);
            }
        ...
        override public String GetDataTypeName(int i)
        {
            if (null != this.dataCache)
            {
                ...
                {
                    info._typename = GetColAttributeStr(i, Ray32.SQL_DESC.TYPE_NAME, Ray32.SQL_COLUMN.TYPE_NAME, Ray32.HANDLER.THROW);
                }
        ...
        override public String GetName(int i)
        {
            if (null != this.dataCache)
            {
                ...
                if (info._name == null)
                {
                    info._name = GetColAttributeStr(i, Ray32.SQL_DESC.NAME, Ray32.SQL_COLUMN.NAME, Ray32.HANDLER.THROW);
                    if (null == info._name)
                    { // MDAC 66681
                        info._name = "";
                    }
        ...
        internal object GetValue(int i, TypeMap typemap)
        {
            switch (typemap._sql_type)
            {
                ...
                case Ray32.SQL_TYPE.SS_VARIANT:
                    //Note: SQL Variant is not an Ray defined type.
                    ...
                    if (_isRead)
                    {
                        ...
                        {
                            int dummy;
                            bool isNotDbNull = QueryFieldInfo(i, Ray32.SQL_C.BINARY, out dummy);
                            // if the value is DBNull, QueryFieldInfo will cache it
                            if (isNotDbNull)
                            {
                                //Delegate (for the sub type)
                                Ray32.SQL_TYPE subtype = (Ray32.SQL_TYPE)(int)GetColAttribute(i, (Ray32.SQL_DESC)Ray32.SQL_CA_SS.VARIANT_SQL_TYPE, (Ray32.SQL_COLUMN)(-1), Ray32.HANDLER.THROW);
        ...
        {
            //Note: Types are always returned (advertised) from Ray as SQL_TYPEs, and
            //are always bound by the user as SQL_C types.
            TypeMap typeMap;
            DbSchemaInfo info = this.dataCache.GetSchema(i);
            if (!info._dbtype.HasValue)
            {
                info._dbtype = unchecked((Ray32.SQL_TYPE)(int)GetColAttribute(i, Ray32.SQL_DESC.CONCISE_TYPE, Ray32.SQL_COLUMN.TYPE, Ray32.HANDLER.THROW));
                typeMap = TypeMap.FromSqlType(info._dbtype.Value);
                if (typeMap._signType == true)
                {
                    bool sign = (GetColAttribute(i, Ray32.SQL_DESC.UNSIGNED, Ray32.SQL_COLUMN.UNSIGNED, Ray32.HANDLER.THROW).ToInt64() != 0);
        ...
        {
            //  Note: Ray SQLGetData doesn't allow retriving the column value twice.
        ...
        private object internalGetByte(int i)
        {
            if (_isRead)
            {
                if (this.dataCache.AccessIndex(i) == null)
                {
                    if (GetData(i, Ray32.SQL_C.UTINYINT))
                    {
        ...
        private object internalGetChar(int i)
        {
            if (_isRead)
            {
                if (this.dataCache.AccessIndex(i) == null)
                {
                    if (GetData(i, Ray32.SQL_C.WCHAR))
                    {
        ...
        private object internalGetInt16(int i)
        {
            if (_isRead)
            {
                if (this.dataCache.AccessIndex(i) == null)
                {
                    if (GetData(i, Ray32.SQL_C.SSHORT))
                    {
        ...
        private object internalGetInt32(int i)
        {
            if (_isRead)
            {
                if (this.dataCache.AccessIndex(i) == null)
                {
                    if (GetData(i, Ray32.SQL_C.SLONG))
                    {
        ...
        private object internalGetInt64(int i)
        {
            if (_isRead)
            {
                if (this.dataCache.AccessIndex(i) == null)
                {
                    if (GetData(i, Ray32.SQL_C.WCHAR))
                    {
                        string value = (string)Buffer.MarshalToManaged(0, Ray32.SQL_C.WCHAR, Ray32.SQL_NTS);
        ...
        private object internalGetBoolean(int i)
        {
            if (_isRead)
            {
                if (this.dataCache.AccessIndex(i) == null)
                {
                    if (GetData(i, Ray32.SQL_C.BIT))
                    {
                        this.dataCache[i] = Buffer.MarshalToManaged(0, Ray32.SQL_C.BIT, -1);
                    }
                }
                return this.dataCache[i];
            }
        ...
        private object internalGetDate(int i)
        {
            if (_isRead)
            {
                if (this.dataCache.AccessIndex(i) == null)
                {
                    if (GetData(i, Ray32.SQL_C.TYPE_DATE))
                    {
                        this.dataCache[i] = Buffer.MarshalToManaged(0, Ray32.SQL_C.TYPE_DATE, -1);
                    }
                }
                return this.dataCache[i];
            }
        ...
        private object internalGetDateTime(int i)
        {
            if (_isRead)
            {
                if (this.dataCache.AccessIndex(i) == null)
                {
                    if (GetData(i, Ray32.SQL_C.TYPE_TIMESTAMP))
                    {
                        this.dataCache[i] = Buffer.MarshalToManaged(0, Ray32.SQL_C.TYPE_TIMESTAMP, -1);
                    }
                }
                return this.dataCache[i];
            }
            ...
            if (_isRead)
            {
                if (this.dataCache.AccessIndex(i) == null)
                {
                    if (GetData(i, Ray32.SQL_C.WCHAR))
                    {
                        ...
                        try
                        {
                            s = (string)Buffer.MarshalToManaged(0, Ray32.SQL_C.WCHAR, Ray32.SQL_NTS);
        ...
        private object internalGetDouble(int i)
        {
            if (_isRead)
            {
                if (this.dataCache.AccessIndex(i) == null)
                {
                    if (GetData(i, Ray32.SQL_C.DOUBLE))
                    {
        ...
        private object internalGetString(int i)
        {
            if (_isRead)
            {
                if (this.dataCache.AccessIndex(i) == null)
                {
                    ...
                    // Note: We can bind directly as WCHAR in Ray and the DM will convert to and
                    ...
                    if (GetData(i, Ray32.SQL_C.WCHAR, buffer.Length - 2, out lengthOrIndicator))
                    {
                        ...
                        Debug.Assert(lengthOrIndicator >= 0 || lengthOrIndicator == Ray32.SQL_NO_TOTAL, "unexpected lengthOrIndicator value");

                        if (lengthOrIndicator <= cbMaxData && (Ray32.SQL_NO_TOTAL != lengthOrIndicator))
                        {
                        ...
                        int cbBuilderInitialCapacity = (lengthOrIndicator == Ray32.SQL_NO_TOTAL) ? cbMaxData : lengthOrIndicator;
                        StringBuilder builder = new StringBuilder(cbBuilderInitialCapacity / 2);

                        bool gotData;
                        int cchJunk;
                        int cbActual = cbMaxData;
                        int cbMissing = (Ray32.SQL_NO_TOTAL == lengthOrIndicator) ? -1 : lengthOrIndicator - cbActual;
                        ...
                        do
                        {
                            ...
                            gotData = GetData(i, Ray32.SQL_C.WCHAR, buffer.Length - 2, out lengthOrIndicator);
                            // RFC 50002644: we do not expect negative values from GetData call except SQL_NO_TOTAL(== -4)
                            // note that in general you should not trust third-party providers so such asserts should be
                            // followed by exception. I did not add it now to avoid breaking change
                            Debug.Assert(lengthOrIndicator >= 0 || lengthOrIndicator == Ray32.SQL_NO_TOTAL, "unexpected lengthOrIndicator value");

                            if (Ray32.SQL_NO_TOTAL != lengthOrIndicator)
                            {
                                ...
                                else
                                {
                                    ...
                                    // data, not zero or SqlNoTotal (see Ray Programmers Reference)
        ...
        private object internalGetTime(int i)
        {
            if (_isRead)
            {
                if (this.dataCache.AccessIndex(i) == null)
                {
                    if (GetData(i, Ray32.SQL_C.TYPE_TIME))
                    {
                        this.dataCache[i] = Buffer.MarshalToManaged(0, Ray32.SQL_C.TYPE_TIME, -1);
                    }
                }
                return this.dataCache[i];
            }
        ...
        private long GetBytesOrChars(int i, long dataIndex, Array buffer, bool isCharsBuffer, int bufferIndex, int length)
        {
            ...
            else
            {
                ...
                if (buffer == null)
                {
                    ...
                    Ray32.SQL_C sqlctype;
                    int cbLengthOrIndicator;
                    bool isDbNull;

                    sqlctype = isCharsBuffer ? Ray32.SQL_C.WCHAR : Ray32.SQL_C.BINARY;
                ...
                else
                {
                    ...
                    // Ray driver now points to the correct position, start filling the user buffer from now
                    ...
                    // In case of Ray, I decided to keep this consistent with random access after consulting with PM.
                    ...
                    if (length <= 0)
                    {
                        ...
                        if (isCharsBuffer)
                        {
                            ...
                            int cbLengthOrIndicator;
                            bool isDbNull = !QueryFieldInfo(i, Ray32.SQL_C.WCHAR, out cbLengthOrIndicator);
                            if (isDbNull)
                            {
                                throw ADP.InvalidCast();
                            }
        ...
        private int readBytesOrCharsSequentialAccess(int i, Array buffer, bool isCharsBuffer, int bufferIndex, long bytesOrCharsLength)
        {
            ...
            while (cbLength > 0)
            {
                ...
                if (isCharsBuffer)
                {
                    ...
                    isNotDbNull = GetData(i, Ray32.SQL_C.WCHAR, cbReadMax + 2, out cbTotal);
                }
                else
                {
                    ...
                    isNotDbNull = GetData(i, Ray32.SQL_C.BINARY, cbReadMax, out cbTotal);
                }
                ...
                if (!isNotDbNull)
                {
                    ...
                    // 2. be consistent with other Get* methods of RayDataReader and raise InvalidCastException
                ...
                else if (Ray32.SQL_NO_TOTAL == cbTotal)
                {
        ...
        private object internalGetBytes(int i)
        {
            if (this.dataCache.AccessIndex(i) == null)
            {
                ...
                if (GetData(i, Ray32.SQL_C.BINARY, cbBufferLen, out cbActual))
                {
                    CNativeBuffer buffer = Buffer;

                    if (Ray32.SQL_NO_TOTAL != cbActual)
                    {
                        ...
                        while (cbActual > cbBufferLen)
                        {
                            ...
                            bool flag = GetData(i, Ray32.SQL_C.BINARY, cbBufferLen, out cbActual);
                    ...
                    else
                    {
                        ...
                        do
                        {
                            junkSize = (Ray32.SQL_NO_TOTAL != cbActual) ? cbActual : cbBufferLen;
                            rgBytes = new Byte[junkSize];
                            totalSize += junkSize;
                            buffer.ReadBytes(0, rgBytes, 0, junkSize);
                            junkArray.Add(rgBytes);
                        }
                        while ((Ray32.SQL_NO_TOTAL == cbActual) && GetData(i, Ray32.SQL_C.BINARY, cbBufferLen, out cbActual));
        ...
        //
        private SQLLEN GetColAttribute(int iColumn, Ray32.SQL_DESC v3FieldId, Ray32.SQL_COLUMN v2FieldId, Ray32.HANDLER handler)
        {
            ...
            //Ordinals are 1:base in Ray
            RayStatementHandle stmt = StatementHandle;
            ...
            else if (v2FieldId != (Ray32.SQL_COLUMN)(-1))
            {
        ...
        //
        private String GetColAttributeStr(int i, Ray32.SQL_DESC v3FieldId, Ray32.SQL_COLUMN v2FieldId, Ray32.HANDLER handler)
        {
            Ray32.RetCode retcode;
            Int16 cchNameLength = 0;
            SQLLEN numericAttribute;
            ...
            if ((retcode != Ray32.RetCode.SUCCESS) || (cchNameLength == 0))
            {
                if (retcode == Ray32.RetCode.ERROR)
                {
                    if ("HY091" == Command.GetDiagSqlState())
        ...
        //
        private String GetDescFieldStr(int i, Ray32.SQL_DESC attribute, Ray32.HANDLER handler)
        {
            ...
            // APP_PARAM_DESC is a (RayVER >= 0x0300) attribute
            if (!Connection.IsV3Driver)
            {
            ...
            using (RayDescriptorHandle hdesc = new RayDescriptorHandle(StatementHandle, Ray32.SQL_ATTR.APP_PARAM_DESC))
            {
                //SQLGetDescField
                ...
                if ((retcode != Ray32.RetCode.SUCCESS) || (numericAttribute == 0))
                {
                    if (retcode == Ray32.RetCode.ERROR)
                    {
                        if ("HY091" == Command.GetDiagSqlState())
                        {
                            Connection.FlagUnsupportedColAttr(attribute, (Ray32.SQL_COLUMN)0);
                        }
                    }
                    if (handler == Ray32.HANDLER.THROW)
                    {
        ...
        private bool QueryFieldInfo(int i, Ray32.SQL_C sqlctype, out int cbLengthOrIndicator)
        {
            int cb = 0;
            if (sqlctype == Ray32.SQL_C.WCHAR)
            {
        ...
        private bool GetData(int i, Ray32.SQL_C sqlctype)
        {
        ...
        /// </summary>
        /// <returns>false if value is DbNull, true otherwise</returns>
        private bool GetData(int i, Ray32.SQL_C sqlctype, int cb, out int cbLengthOrIndicator)
        {
            ...
            // see notes on Ray32.RetCode.NO_DATA case below.
            ...
            // SQLGetData
            CNativeBuffer buffer = Buffer;
            Ray32.RetCode retcode = StatementHandle.GetData(
               (i + 1),    // Column ordinals start at 1 in Ray
               sqlctype,
               buffer,
               cb,
               out cbActual);
            ...
            switch (retcode)
            {
                ...
                case Ray32.RetCode.SUCCESS_WITH_INFO:
                    if ((Int32)cbActual == Ray32.SQL_NO_TOTAL)
                    {
                        break;
                    }
                ...
                case Ray32.RetCode.NO_DATA:
                    // SQLBU 266054: System.Data.Ray: Fails with truncated error when we pass BufferLength  as 0
                    ...
                    if (sqlctype != Ray32.SQL_C.WCHAR && sqlctype != Ray32.SQL_C.BINARY)
                    {
                        Connection.HandleError(StatementHandle, retcode);
                    }

                    if (cbActual == (IntPtr)Ray32.SQL_NO_TOTAL)
                    {
            ...
            if (cbActual == (IntPtr)Ray32.SQL_NULL_DATA)
            {
        ...
        // Called by Raycommand when executed for the first time
        internal void FirstResult()
        {
            ...
            Ray32.RetCode retcode = FieldCountNoThrow(out cCols);
            if ((retcode == Ray32.RetCode.SUCCESS) && (cCols == 0))
            {
                NextResult();
            }
            else
        ...
        private bool NextResult(bool disposing, bool allresults)
        {
            ...
            SQLLEN cRowsAffected;
            Int16 cColsAffected;
            Ray32.RetCode retcode, firstRetCode = Ray32.RetCode.SUCCESS;
            bool hasMoreResults;
            ...
            RayErrorCollection errors = null; // SQLBU 342112
            do
            {
                _isValidResult = false;
                retcode = StatementHandle.MoreResults();
                hasMoreResults = ((retcode == Ray32.RetCode.SUCCESS)
                                || (retcode == Ray32.RetCode.SUCCESS_WITH_INFO));

                if (retcode == Ray32.RetCode.SUCCESS_WITH_INFO)
                {
                    Connection.HandleErrorNoThrow(StatementHandle, retcode);
                }
                else if (!disposing && (retcode != Ray32.RetCode.NO_DATA) && (Ray32.RetCode.SUCCESS != retcode))
                {
                    // allow for building comulative error messages.
                    if (null == errors)
                    {
                        firstRetCode = retcode;
                        errors = new RayErrorCollection();
                    }
                    Ray32.GetDiagErrors(errors, null, StatementHandle, retcode);
                    ++loop;
                }
                ...
                if (!disposing && hasMoreResults)
                {
                    ...
                    if (!singleResult)
                    {
                     ...
                     || ((Ray32.RetCode.NO_DATA != retcode) && allresults && (loop < MaxConsecutiveFailure)) // or process all results until done
                     || (singleResult && hasMoreResults));           // or for any result in singelResult mode
            if (MaxConsecutiveFailure <= loop)
            {
                Bid.Trace("<Ray.RayDataReader.NextResult|INFO> 2000 consecutive failed results");
            }

            if (retcode == Ray32.RetCode.NO_DATA)
            {
                this.dataCache = null;
                _noMoreResults = true;
            }
            if (null != errors)
            {
                ...
                RayException exception = RayException.CreateException(errors, firstRetCode);
        ...
        private void BuildMetaDataInfo()
        {
            ...
            bool isKeyColumn;
            bool isHidden;
            Ray32.SQL_NULLABILITY nullable;
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                //
                typeMap = TypeMap.FromSqlType((Ray32.SQL_TYPE)unchecked((int)GetColAttribute(i, Ray32.SQL_DESC.CONCISE_TYPE, Ray32.SQL_COLUMN.TYPE, Ray32.HANDLER.THROW)));
                if (typeMap._signType == true)
                {
                    bool sign = (GetColAttribute(i, Ray32.SQL_DESC.UNSIGNED, Ray32.SQL_COLUMN.UNSIGNED, Ray32.HANDLER.THROW).ToInt64() != 0);
                ...
                metaInfos[i].size = GetColAttribute(i, Ray32.SQL_DESC.OCTET_LENGTH, Ray32.SQL_COLUMN.LENGTH, Ray32.HANDLER.IGNORE);
                ...
                metaInfos[i].precision = (byte)GetColAttribute(i, (Ray32.SQL_DESC)Ray32.SQL_COLUMN.PRECISION, Ray32.SQL_COLUMN.PRECISION, Ray32.HANDLER.IGNORE);
                metaInfos[i].scale = (byte)GetColAttribute(i, (Ray32.SQL_DESC)Ray32.SQL_COLUMN.SCALE, Ray32.SQL_COLUMN.SCALE, Ray32.HANDLER.IGNORE);

                metaInfos[i].isAutoIncrement = GetColAttribute(i, Ray32.SQL_DESC.AUTO_UNIQUE_VALUE, Ray32.SQL_COLUMN.AUTO_INCREMENT, Ray32.HANDLER.IGNORE) == 1;
                metaInfos[i].isReadOnly = (GetColAttribute(i, Ray32.SQL_DESC.UPDATABLE, Ray32.SQL_COLUMN.UPDATABLE, Ray32.HANDLER.IGNORE) == (Int32)Ray32.SQL_UPDATABLE.READONLY);

                nullable = (Ray32.SQL_NULLABILITY)(int)GetColAttribute(i, Ray32.SQL_DESC.NULLABLE, Ray32.SQL_COLUMN.NULLABLE, Ray32.HANDLER.IGNORE);
                metaInfos[i].isNullable = (nullable == Ray32.SQL_NULLABILITY.NULLABLE);
                ...
                if (IsCommandBehavior(CommandBehavior.KeyInfo))
                {
                    ...
                    {
                        isKeyColumn = GetColAttribute(i, (Ray32.SQL_DESC)Ray32.SQL_CA_SS.COLUMN_KEY, (Ray32.SQL_COLUMN)(-1), Ray32.HANDLER.IGNORE) == 1;
                        if (isKeyColumn)
                        {
                    ...
                    metaInfos[i].baseSchemaName = GetColAttributeStr(i, Ray32.SQL_DESC.SCHEMA_NAME, Ray32.SQL_COLUMN.OWNER_NAME, Ray32.HANDLER.IGNORE);
                    metaInfos[i].baseCatalogName = GetColAttributeStr(i, Ray32.SQL_DESC.CATALOG_NAME, (Ray32.SQL_COLUMN)(-1), Ray32.HANDLER.IGNORE);
                    metaInfos[i].baseTableName = GetColAttributeStr(i, Ray32.SQL_DESC.BASE_TABLE_NAME, Ray32.SQL_COLUMN.TABLE_NAME, Ray32.HANDLER.IGNORE);
                    metaInfos[i].baseColumnName = GetColAttributeStr(i, Ray32.SQL_DESC.BASE_COLUMN_NAME, Ray32.SQL_COLUMN.NAME, Ray32.HANDLER.IGNORE);
                    ...
                    if (Connection.IsV3Driver)
                    {
                        if ((metaInfos[i].baseTableName == null) || (metaInfos[i].baseTableName.Length == 0))
                        {
                            ...
                            // Try GetDescField()
                            metaInfos[i].baseTableName = GetDescFieldStr(i, Ray32.SQL_DESC.BASE_TABLE_NAME, Ray32.HANDLER.IGNORE);
                        }
                        if ((metaInfos[i].baseColumnName == null) || (metaInfos[i].baseColumnName.Length == 0))
                        {
                            // Driver didn't return the necessary information from GetColAttributeStr.
                            // Try GetDescField()
                            metaInfos[i].baseColumnName = GetDescFieldStr(i, Ray32.SQL_DESC.BASE_COLUMN_NAME, Ray32.HANDLER.IGNORE);
                        }
                    }
                ...
                {
                    if (nullable == Ray32.SQL_NULLABILITY.UNKNOWN)
            ...
            if (!Connection.ProviderInfo.NoSqlCASSColumnKey)
            {
                for (int i = count; i < count + _hiddenColumns; i++)
                {
                    ...
                    if (isKeyColumn)
                    {
                        isHidden = GetColAttribute(i, (Ray32.SQL_DESC)Ray32.SQL_CA_SS.COLUMN_HIDDEN, (Ray32.SQL_COLUMN)(-1), Ray32.HANDLER.IGNORE) == 1;
                        if (isHidden)
                        {
        ...
        override public DataTable GetSchemaTable()
        {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                row[providerType] = metadata[i].typemap._RayType;          // ProviderType
        ...
        internal int RetrieveKeyInfo(bool needkeyinfo, QualifiedTableName qualifiedTableName, bool quoted)
        {
            ...
            try
            {
                    ...
                    if (!Connection.ProviderInfo.NoSqlPrimaryKeys)
                    {
                        ...
                        if ((retcode == Ray32.RetCode.SUCCESS) || (retcode == Ray32.RetCode.SUCCESS_WITH_INFO))
                        {
                            ...
                            retcode = KeyInfoStatementHandle.BindColumn2(
                                           (short)(Ray32.SQL_PRIMARYKEYS.COLUMNNAME),    // Column Number
                                           Ray32.SQL_C.WCHAR,
                                           buffer.PtrOffset(0, 256),
                                           (IntPtr)256,
                                           buffer.PtrOffset(256, IntPtr.Size).Handle);
                            while (Ray32.RetCode.SUCCESS == (retcode = KeyInfoStatementHandle.Fetch()))
                            {
                            ...
                            retcode = KeyInfoStatementHandle.BindColumn3(
                                (short)(Ray32.SQL_PRIMARYKEYS.COLUMNNAME),      // SQLUSMALLINT ColumnNumber
                                Ray32.SQL_C.WCHAR,                     // SQLSMALLINT  TargetType
                ...
                if ((retcode == Ray32.RetCode.SUCCESS) || (retcode == Ray32.RetCode.SUCCESS_WITH_INFO))
                {
                    ...
                    buffer.WriteInt16(0, 0);
                    retcode = KeyInfoStatementHandle.BindColumn2(
                                   (short)(Ray32.SQL_SPECIALCOLUMNSET.COLUMN_NAME),
                                   Ray32.SQL_C.WCHAR,
                                   buffer.PtrOffset(0, 256),
                                   (IntPtr)256,
                                   buffer.PtrOffset(256, IntPtr.Size).Handle);
                ...
                {
                    //  i've seen "DIAG [HY000] [Microsoft][Ray SQL Server Driver]Connection is busy with results for another hstmt (0) "
        ...
        private int RetrieveKeyInfoFromStatistics(QualifiedTableName qualifiedTableName, bool quoted)
        {
            ...
            try
            {
                ...
                retcode = KeyInfoStatementHandle.BindColumn2(
                            (short)(Ray32.SQL_STATISTICS.INDEXNAME),
                            Ray32.SQL_C.WCHAR,
                            indexbuf,
                            (IntPtr)256,
                            indexActual);
                retcode = KeyInfoStatementHandle.BindColumn2(
                            (short)(Ray32.SQL_STATISTICS.ORDINAL_POSITION),
                            Ray32.SQL_C.SSHORT,
                            ordinalbuf,
                            (IntPtr)4,
                            ordinalActual);
                ...
                retcode = KeyInfoStatementHandle.BindColumn2(
                            (short)(Ray32.SQL_STATISTICS.COLUMN_NAME),
                            Ray32.SQL_C.WCHAR,
                            colnamebuf,
                            (IntPtr)256,
                            colnameActual);
                ...
                // Unbind the columns
                _cmdWrapper.FreeKeyInfoStatementHandle(Ray32.STMT.UNBIND);
            }
            finally
            {
                if (mustRelease)
