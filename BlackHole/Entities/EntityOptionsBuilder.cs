...
namespace BlackHole.Entities
{
    ...
    public class EntityOptionsBuilder<T>
    {
        ...
        /// <typeparam name="tKey"></typeparam>
        /// <param name="primaryKey">Property of the entity</param>
        /// <param name="valueGenerator">Function that generates unique value</param>
        /// <returns>Entity Settings Object</returns>
        public EntitySettings<T> SetPrimaryKey<tKey>(Expression<Func<T, tKey>> primaryKey, Func<tKey> valueGenerator) where tKey : IComparable<tKey>
        {
            EntitySettings<T> settings = new(false);
            if (primaryKey.Body is MemberExpression pkMember)
            {
    ...
    public class EntitySettings<T>
    {
        ...
        internal List<DefaultValueProperty> DefaultValues { get; set; } = new();
    ...
    public static class PKExtensions
    {
        ...
        /// <param name="valueGenerator">lass that inherits from IBHValueGenerator</param>
        /// <returns>Entity Settings Object</returns>
        public static EntitySettings<T> CompositeKey<T, tKey>(this EntitySettings<T> settings, Expression<Func<T, tKey>> primaryKey, Func<tKey> valueGenerator)
         where tKey : IComparable<tKey>
        {
            if (!settings.LockedPK)
            {
            ...
            where tKey : IComparable<tKey>
        {
            if (primaryKey.Body is MemberExpression pkMember)
            {
                DefaultValueProperty? ExistingGenerator = settings.DefaultValues.FirstOrDefault(x => x.PropertyName == pkMember.Member.Name);

                if (ExistingGenerator != null)
                {
                    settings.DefaultValues.Remove(ExistingGenerator);
                }

                settings.DefaultValues.Add(new DefaultValueProperty { PropertyName = pkMember.Member.Name, ValueIfNull = Value });
            }
