...
namespace TestBench
{
    internal class ExpressionsData
    {
        ...
        public bool IsReversed
        {
            get
            {
                return LeftMember == null && RightMember != null && MethodData.Count == 0;
            }
        }
    }
    ...
    public static class BHExpressionParser
    {
        ...
        private static BHExpressionPart MapEdge(this ExpressionsData item)
        {
            ...
            return new BHExpressionPart
            {
                ...
                RightName = item.IsReversed ? null : item.RightMember?.ToString().Split(".")[1],
                Value = item.MemberValue,
                ExpType = item.IsReversed ? item.OperationType.ReverseOperator() : item.OperationType,
                OpenParenthesis = item.OpenParenthesis,
                CLoseParenthesis = item.CloseParenthesis
            };
        }

        public static ExpressionType ReverseOperator( this ExpressionType expType)
        {
        ...
        public static BHExpressionPart[] ParseExpression<T>(this T entry ,Expression<Func<T, bool>> completeExpression)
        {
